...
import type { DfuInterfaceSettings } from "../../src/dfu";
import {
  STATUS_OK,
  dfuIDLE,
  dfuDNLOAD_IDLE,
...
export function createStateResponse(state: number): DataView {
...
export function scriptedTransferIn(
  responses: Array<{ status: string; data: DataView }>
) {
  let idx = 0;
...
export function downloadPollSequence(): Array<{
  status: string;
  data: DataView;
}> {
  return [
...
export interface MockUSBDeviceOptions {
  ...
  productName?: string;
  serialNumber?: string;
  dfuseDescriptor?: string;
  interfaceProtocol?: number;
}

export function createMockUSBDevice(opts: MockUSBDeviceOptions = {}) {
  const alternate = {
    alternateSetting: 0,
    interfaceClass: 0xfe,
...
export function createMockDfuSettings(
  device: ReturnType<typeof createMockUSBDevice>,
  opts: { dfuseDescriptor?: string; interfaceProtocol?: number } = {}
  ...
  return {
    ...
    interface: intf as unknown as USBInterface,
    alternate: alt as unknown as USBAlternateInterface,
