...
describe("parseMemoryDescriptor", () => {
    it("parses standard STM32H750 descriptor", () => {
        ...
        const desc = "@Internal Flash  /0x08000000/01*128Ke";
    ...
    it("parses multi-sector descriptor", () => {
        const desc = "@Internal Flash  /0x08000000/04*016Kg,01*064Kg,07*128Kg";
        const info = parseMemoryDescriptor(desc);
        expect(info.name).toBe("Internal Flash");
    ...
    it("handles byte multipliers", () => {
        ...
        expect(parseMemoryDescriptor(descK).segments[0].sectorSize).toBe(64 * 1024);
        const descM = "@Flash/0x00000000/01*002Me";
        expect(parseMemoryDescriptor(descM).segments[0].sectorSize).toBe(2 * 1048576);
    });
    ...
    it("throws for invalid descriptor (no @)", () => {
        expect(() => parseMemoryDescriptor("Internal Flash/0x08000000/01*128Ke")).toThrow("Not a DfuSe memory descriptor");
    });
    it("throws for invalid descriptor (no /)", () => {
        expect(() => parseMemoryDescriptor("@InternalFlash")).toThrow("Not a DfuSe memory descriptor");
    });
