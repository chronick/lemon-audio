...
function makeFlashDevice(opts: { dfuseDescriptor?: string } = {}) {
...
function queueStatus(
  usb: ReturnType<typeof createMockUSBDevice>,
  status: number,
  pollTimeout: number,
  state: number
) {
...
describe("DfuDevice.flash (plain DFU)", () => {
  it("flashes firmware with correct block sequence", async () => {
    ...
    const dnloadCalls = usb.controlTransferOut.mock.calls.filter(
      (call: unknown[]) => (call[0] as { request: number }).request === 0x01
    );
  ...
  it("reports progress during download", async () => {
    ...
    const progressCalls = (dfu.logProgress as ReturnType<typeof vi.fn>).mock
      .calls;
...
describe("DfuDevice.flash (DfuSe)", () => {
  ...
  it("erases then downloads with SET_ADDRESS commands", async () => {
    ...
    expect(dfu.logInfo).toHaveBeenCalledWith(
      expect.stringContaining("Erasing")
    );
    expect(dfu.logInfo).toHaveBeenCalledWith(
      expect.stringContaining("Wrote 2048 bytes")
    );
  });
});
