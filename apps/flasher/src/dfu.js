...
export class DfuDevice {
    ...
    async open() {
        ...
        // Robustness fix: if the DfuSe memory map wasn't available from the
        // interface name at construction (some browsers/devices don't surface
        // `alt.interfaceName`), recover it now. Without it `isDfuSe` is false and
        // the flasher falls back to PLAIN DFU, which an STM32 DfuSe bootloader
        // misreads (block 0/1 are interpreted as commands) → a GETSTATUS stall.
        if (this.memoryInfo === null) {
            await this.recoverMemoryInfo();
        }
    }
    async recoverMemoryInfo() {
        ...
        catch (e) { this.logWarning(`String-descriptor recovery failed: ${e}`); }
        // 3) ST system-bootloader fallback: assume the internal-flash layout.
        if (this.device.vendorId === 0x0483) {
            ...
            this.logWarning("No DfuSe descriptor surfaced — assuming STM32 internal flash (16*128K @ 0x08000000)");
        }
    }
    async readInterfaceStringIndex() {
        ...
        while (off + 9 <= data.byteLength) {
            ...
            if (bType === 0x04) { // INTERFACE descriptor
                ...
                if (ifNum === this.intfNumber && altSet === want)
                    return iInterface;
            }
            off += bLength;
        }
    ...
    async readStringDescriptor(index) {
