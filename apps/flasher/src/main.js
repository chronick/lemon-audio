import "./style.css";
import { DfuDevice, findDeviceDfuInterfaces, dfuERROR, } from "./dfu";
import { resolveRelease, allControls, } from "./catalog";
// STM32 DFU VID/PID
...
async function loadFirmware() {
    ...
    firmware = resolved.filter((fw) => fw !== null);
    renderCatalog();
}
// --- Render ---
function renderCatalog() {
    ...
    firmwareListEl.querySelectorAll(".firmware-card").forEach((card) => {
        card.addEventListener("click", () => {
            selectFirmware(card.dataset.id);
        });
    });
}
...
flashBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        const xferSize = dfuProperties?.TransferSize ?? 2048;
        const manifestationTolerant = dfuProperties?.ManifestationTolerant ?? false;
